...
void bstTOhash (  node_t *rp,  ST st  )
{
  data_t data;
  Item item;
  ...
  STinsert (st, item);
  bstTOhash (rp->right, st);

  return;
}

void listAndBST (link list, node_t *rp){

    data_t data;
    int level;
while(list!=NULL){

   data=dataset(KEYget(ITEMget(list)), getField2I(ITEMget(list)), getField3I(ITEMget(list)), getField4I(ITEMget(list)));
  level=levelBST(rp, data);
  ITEMshow(ITEMget(list), stdout);
  printf(" Level: %d \n", level);
  list=NextPLink(list);
}
return ;
...
int levelBST(node_t* rp, data_t data){
int lev=0;
  while (rp != NULL) {
    if (compare(rp->val, data) == 0)
      return (lev);
