...
struct symboltable {
 ...
 Item   ITEMget(link list){
 return list->item;
 }

link NextPLink(link list){
return list->next;
}
...
void visitR(link h, link z, FILE* fp) {
  if (h == z)
    return;
  ITEMshow(h->item, fp); //modify functions for files
  visitR(h->next, z, fp); //modify functions for files
  }
...
void STdisplay(ST st, FILE* fp)  {
  ...
  for (i=0; i < st->M; i++) {
    ...
    visitR(st->heads[i], st->z, fp); //modify functions for files
    fprintf(fp, "\n");
  }
}
...
for(i=0; i<st->M; i++){
       if(st->heads[i]!=st->z){
            st->heads[i]=st->heads[i]->next;
        }
    while(st->heads[i]!=st->z){
        tmp=malloc(sizeof*tmp);
        tmp->item=st->heads[i]->item;
        tmp->next=head;
        head=tmp;
        st->heads[i]=st->heads[i]->next;
