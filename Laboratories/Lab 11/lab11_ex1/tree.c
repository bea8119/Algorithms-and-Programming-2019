...
#if FIND
  static Item findDeleteMax1 (node_t **);
#endif
#if !FIND
  static node_t *findDeleteMax2 (Item *, node_t *);
#endif

Item getData (node_t *node)
{
  return (node->val);
}
...
node_t *searchI (node_t *rp,  Item data  )
{
  while (rp != NULL) {
...
node_t *searchR (node_t *rp,  Item data  )
{
...
node_t *insert (  node_t *rp,  Item data)
{
  node_t *p;
...
void
writeTree (FILE *fp,  node_t *rp,  int modo)
{
  if (rp == NULL) {
    return;
  }
