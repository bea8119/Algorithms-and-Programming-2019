...
int main (int argc, char *argv[]) {
  ...
  graph_attribute_init (g);
  i=graph_bfs_count(g, src );
  printf("%d\n", i);


    graph_attribute_init (g);
        i=graph_dfs_distance(g, src );
          printf("\n\nMax distance of a node from source In DST:\n");
        printf("%d\n", i);
int T,F,B,C;
T=F=B=C=0;
 graph_attribute_init (g);
 graph_dfs_type(g, src, &T, &F, &B, &C);
 printf("\n\n Number of edges by type:\n\tTrees: %d\n\tForward : %d\n\tBackward: %d\n\tCross: %d\n", T,F,B,C);

 graph_attribute_init (g);
        i=graph_dfs_depth(g, src, 0 );
          printf("\n\nDepth of the visit tree of DFS:\n");
        printf("%d\n", i);
